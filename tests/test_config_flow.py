...
async def test_reconfigure_flow_success(
    ...
    """Reconfigure updates the host/credentials of the same device in place."""
    entry = _existing_entry()
    entry.add_to_hass(hass)
    ...
    assert entry.data[CONF_HOST] == "http://192.168.1.1"
    assert entry.data[CONF_PASSWORD] == "new-password"
...
async def test_reconfigure_flow_wrong_device(
    ...
    """Reconfiguring against a router with a different MAC is rejected."""
    entry = _existing_entry()  # unique_id 00:11:22:00:00:01
    entry.add_to_hass(hass)
    mock_flow_api.router_info.return_value = {
